...
@triton.jit
def fuse_scale_shift_select01_kernel_blc_opt(
    x_ptr,
    shift0_ptr,
    scale0_ptr,
    shift1_ptr,
...
def fuse_scale_shift_gate_select01_kernel(
    ...
    B, L, C = x.shape
    output = torch.empty_like(x)
    gate_out = torch.empty((B, L, C), device=x.device, dtype=x.dtype)

    if scale0.dim() != 2 or shift0.dim() != 2 or gate0.dim() != 2 or scale1.dim() != 2 or shift1.dim() != 2 or gate1.dim() != 2:
...
def fuse_scale_shift_select01_kernel(
    x: torch.Tensor,
    scale0: torch.Tensor,
    shift0: torch.Tensor,
    scale1: torch.Tensor,
    ...
    if scale0.dim() != 2 or shift0.dim() != 2 or scale1.dim() != 2 or shift1.dim() != 2:
        raise ValueError("scale0/shift0/scale1/shift1 must be 2D [B, C]")
    if index.dim() != 2:
        raise ValueError("index must be 2D [B, L]")

    grid = (triton.cdiv(L, block_l), triton.cdiv(C, block_c), B)
    fuse_scale_shift_select01_kernel_blc_opt[grid](
        x,
        shift0,
        scale0,
        shift1,
